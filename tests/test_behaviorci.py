...
class TestSerialization:
    ...
    def test_fails_on_datetime(self):
        ...
        assert (
            ...
            or "not json-serializable" in str(exc_info.value).lower()
        )
...
class TestComparator:
    ...
    def test_lexical_must_contain_pass(self, temp_comparator):
        ...
        passed, missing, forbidden = temp_comparator.check_lexical(
            "This is a refund request", must_contain=["refund"], must_not_contain=[]
        )
    ...
    def test_lexical_must_contain_fail(self, temp_comparator):
        ...
        passed, missing, forbidden = temp_comparator.check_lexical(
            "This is a request", must_contain=["refund"], must_not_contain=[]
        )
    ...
    def test_lexical_must_not_contain_fail(self, temp_comparator):
        ...
        passed, missing, forbidden = temp_comparator.check_lexical(
            "This contains error message", must_contain=[], must_not_contain=["error"]
        )
        assert passed is False
    ...
    def test_record_and_compare_same(self, temp_comparator):
        ...
        temp_comparator.record_snapshot(
            behavior_id="test", input_json='{"args": []}', output_text="test output"
        )
    ...
    def test_compare_different_output_fails(self, temp_comparator):
        ...
        # Record snapshot
        temp_comparator.record_snapshot(
            behavior_id="test", input_json='{"args": []}', output_text="original output"
        )
    ...
    def test_lexical_override_high_similarity(self, temp_comparator):
        ...
        assert result.passed is False
        assert "Missing required" in result.message or "guarantee" in str(
            result.missing_must_contain
        )
    ...
    def test_variance_aware_threshold(self):
        ...
        assert (
            effective_high < base
        ), f"High variance should lower threshold below {base}, got {effective_high}"
