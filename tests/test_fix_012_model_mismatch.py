...
def test_model_mismatch_raises_error():
    ...
    assert "model-A" in error_message, f"Error should mention stored model 'model-A': {error_message}"
    assert "model-B" in error_message, f"Error should mention current model 'model-B': {error_message}"
    assert "incomparable" in error_message.lower() or "different vector" in error_message.lower(), \
...
def test_model_mismatch_error_details():
    ...
    embedder_a = MockEmbedderA()
    comparator_a = Comparator(storage, embedder_a)
    comparator_a.record_snapshot(
        behavior_id="test_details",
        input_json='{}',
        output_text="test"
    )
    ...
    assert "--behaviorci-model" in error.message or "specify the original model" in error.message.lower()
