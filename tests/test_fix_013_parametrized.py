...
def test_parametrized_no_cross_contamination():
    ...
    for input_text in ["hello", "world", "test"]:
        ...
        assert result.passed is True, \
            f"Comparison failed for '{input_text}': {result.message}"
...
def test_parametrized_integration():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with open(test_file, "w") as f:
            f.write('''
import sys
sys.path.insert(0, "''' + os.path.dirname(os.path.dirname(__file__)).replace('\\', '\\\\') + '''")
sys.path.insert(0, "''' + os.path.join(os.path.dirname(__file__), "examples").replace('\\', '\\\\') + '''")
