...
def test_high_variance_lowers_threshold():
    ...
    assert threshold >= 0.5, \
        f"Threshold should not go below 0.5, got {threshold}"
...
def test_minimum_threshold_floor():
    ...
    assert threshold == 0.5, \
        f"Threshold should be clamped to 0.5 minimum, got {threshold}"
...
def test_insufficient_history_uses_base():
    ...
    assert abs(threshold - base_threshold) < 0.001, \
...
def test_variance_vs_max_bug():
    ...
    assert threshold < 0.80, \
        f"Threshold {threshold} should be < 0.80 (proves max() bug is fixed)"
