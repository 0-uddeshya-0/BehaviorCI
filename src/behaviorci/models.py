...
class BehaviorConfig(BaseModel):
    ...
    behavior_id: str = Field(..., description="Logical behavior identifier")
    threshold: float = Field(default=0.85, ge=0.0, le=1.0, description="Minimum similarity threshold")
...
class ComparisonResult(BaseModel):
    ...
    missing_must_contain: List[str] = Field(default_factory=list, description="Missing required substrings")
    found_must_not_contain: List[str] = Field(default_factory=list, description="Found forbidden substrings")
