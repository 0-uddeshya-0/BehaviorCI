...
from .exceptions import BehaviorCIError, SerializationError
...
def pytest_configure(config):
    ...
    config.behaviorci_enabled = (
        ...
        config.getoption("--behaviorci-update")
    )
...
def pytest_runtest_makereport(item, call):
    ...
    storage = Storage(item.config.behaviorci_db_path)
...
def pytest_terminal_summary(terminalreporter, exitstatus, config):
    ...
    storage = Storage(config.behaviorci_db_path)
