...
def pytest_runtest_call(item):
    ...
    CRITICAL-001 FIX: Uses hookwrapper=True with yield to let pytest run
    the test exactly once. Previous implementation called item.runtest() 
    manually, causing double execution (pytest's runner also calls it).
    
    WHY: pytest_runtest_call is NOT a firstresult hook. Multiple implementations
    can exist and all will be called. Using hookwrapper=True with yield lets
    pytest's internal runner execute the test, then we capture the result.
    
    VERIFIED BY: tests/test_critical_001_single_execution.py
    """
    # Let pytest run the test normally (once)
    outcome = yield
    
    # After test completes, capture result for behavior tests
    if CONFIG_KEY not in item.stash:
        return  # Not a behavior test
    
    # Check if test raised an exception
    if outcome.excinfo is not None:
        # Test failed with exception - don't try to capture result
        return
